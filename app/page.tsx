...
function MarketView({ game, update }: { game: GameState; update: (next: GameState) => void }) {
  ...
  return (
    <>
      <SectionIntro eyebrow="JANELA DE TRANSFERÊNCIAS" title="Mercado"><span className="inline-budget">Orçamento disponível: <strong>{formatMoney(game.transferBudget)}</strong></span></SectionIntro>
...
function CareerModal({ game, careers, onClose, onSelect, onCreate, onDelete, onImport, onSync }: { game: GameState; careers: GameState[]; onClose:()=>void; onSelect:(id:string)=>void; onCreate:(manager:string,club:string,name:string)=>void; onDelete:(id:string)=>void; onImport:(game:GameState)=>void; onSync:(game:GameState)=>void }) {
  const [creating,setCreating]=useState(false); const [manager,setManager]=useState(game.managerName); const [name,setName]=useState("Nova carreira"); const [club,setClub]=useState("florianopolis"); const [url,setUrl]=useState(""); const [realTeam,setRealTeam]=useState(userClub(game).name); const [status,setStatus]=useState(""); const fileRef=useRef<HTMLInputElement>(null);
...
export default function Home() {
  ...
  const createCareer=(manager:string,club:string,name:string)=>{const next={...createNewGame(manager,club,name),id:`career-${Date.now()}`};setCareers((items)=>[...items,next]);setActiveId(next.id);setCareerModal(false);setSection("Visão geral");};
