...
function CompetitionView({ game }: { game: GameState }) {
  ...
  const knockoutFixtures=competitionFixtures.filter((fixture)=>!fixture.stage.startsWith("Grupo ")&&fixture.stage!=="Liga");
  const serieDGroups=competition.id==="BRA-D"?[...new Set(competitionFixtures.filter((fixture)=>fixture.stage.startsWith("Grupo ")).map((fixture)=>fixture.stage))].map((name)=>{const fixtures=competitionFixtures.filter((fixture)=>fixture.stage===name),ids=[...new Set(fixtures.flatMap((fixture)=>[fixture.homeId,fixture.awayId]))];return{name,table:calculateStandings(ids.map((id)=>clubById(game,id)),fixtures)};}):[];
  ...
  return (
    <>
      ...
      {competition.id==="BRA-D"?<><section className="series-d-groups">{serieDGroups.map((group)=><article className="group-panel panel" key={group.name}><div className="panel-title"><span>{group.name.toUpperCase()}</span><small>4 AVANÇAM</small></div><div className="group-standing header"><span>#</span><span>CLUBE</span><span>J</span><span>SG</span><b>PTS</b><em>DESTINO</em></div>{group.table.map((row,index)=>{const club=clubById(game,row.clubId);return <div className={`group-standing ${row.clubId===game.userClubId?"is-us":""}`} key={row.clubId}><span>{index+1}</span><span><Crest club={club} small/><strong>{club.name}</strong></span><span>{row.played}</span><span>{row.gd}</span><b>{row.points}</b><em>{index<4?"CLASSIFICA":"—"}</em></div>;})}</article>)}</section>{knockoutFixtures.length>0&&fixtureFeed(knockoutFixtures)}</>:(competition.type==="league"||competition.type==="state") ? <><article className="full-standings panel">
