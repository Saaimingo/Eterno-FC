...
function CompetitionView({ game }: { game: GameState }) {
  ...
  const knockoutFixtures=competitionFixtures.filter((fixture)=>!fixture.stage.startsWith("Grupo ")&&fixture.stage!=="Liga"&&fixture.stage!=="Fase de grupos");
  const groupTables=competition.groups?Object.entries(competition.groups).map(([name,ids])=>({name,table:calculateStandings(ids.map((id)=>clubById(game,id)),competitionFixtures.filter((fixture)=>fixture.stage==="Fase de grupos"))})):competition.id==="BRA-D"?[...new Set(competitionFixtures.filter((fixture)=>fixture.stage.startsWith("Grupo ")).map((fixture)=>fixture.stage))].map((name)=>{const fixtures=competitionFixtures.filter((fixture)=>fixture.stage===name),ids=[...new Set(fixtures.flatMap((fixture)=>[fixture.homeId,fixture.awayId]))];return{name,table:calculateStandings(ids.map((id)=>clubById(game,id)),fixtures)};}):[];
