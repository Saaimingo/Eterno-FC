...
function CompetitionView({ game }: { game: GameState }) {
  ...
  const knockoutFixtures=competitionFixtures.filter((fixture)=>!fixture.stage.startsWith("Grupo ")&&fixture.stage!=="Liga"&&fixture.stage!==(competition.groupStage??"Fase de grupos"));
  const groupTables=competition.groups?Object.entries(competition.groups).map(([name,ids])=>({name,table:calculateStandings(ids.map((id)=>clubById(game,id)),competitionFixtures.filter((fixture)=>fixture.stage===(competition.groupStage??"Fase de grupos")))})):competition.id==="BRA-D"?[...new Set(competitionFixtures.filter((fixture)=>fixture.stage.startsWith("Grupo ")).map((fixture)=>fixture.stage))].map((name)=>{const fixtures=competitionFixtures.filter((fixture)=>fixture.stage===name),ids=[...new Set(fixtures.flatMap((fixture)=>[fixture.homeId,fixture.awayId]))];return{name,table:calculateStandings(ids.map((id)=>clubById(game,id)),fixtures)};}):[];
  const fixtureFeed=(fixtures:typeof competitionFixtures)=><section className="cup-bracket panel"><div className="cup-stage-head"><span>FASES E RESULTADOS</span><strong>{competition.complete?"ENCERRADA":competition.currentStage.toUpperCase()}</strong></div>{fixtures.map((fixture)=>{const home=clubById(game,fixture.homeId),away=clubById(game,fixture.awayId);return <div className={`cup-fixture ${fixture.homeId===game.userClubId||fixture.awayId===game.userClubId?"is-us":""}`} key={fixture.id}><span>{fixture.stage}{fixture.leg&&<b> • JOGO {fixture.leg}</b>}<small>{formatGameDate(fixture.date)}</small></span><div><strong>{home.name}</strong><Crest club={home} small/></div><b>{fixture.played?`${fixture.homeGoals} — ${fixture.awayGoals}`:"x"}</b><div><Crest club={away} small/><strong>{away.name}</strong></div></div>;})}</section>;
  return (
    <>
      ...
      {groupTables.length?<><section className="series-d-groups">{groupTables.map((group)=><article className="group-panel panel" key={group.name}><div className="panel-title"><span>{group.name.toUpperCase()}</span><small>{competition.groupAdvancing??(competition.id==="BRA-D"?4:2)} AVANÇAM</small></div><div className="group-standing header"><span>#</span><span>CLUBE</span><span>J</span><span>SG</span><b>PTS</b><em>DESTINO</em></div>{group.table.map((row,index)=>{const club=clubById(game,row.clubId),advancing=index<(competition.groupAdvancing??(competition.id==="BRA-D"?4:2));return <div className={`group-standing ${row.clubId===game.userClubId?"is-us":""}`} key={row.clubId}><span>{index+1}</span><span><Crest club={club} small/><strong>{club.name}</strong></span><span>{row.played}</span><span>{row.gd}</span><b>{row.points}</b><em>{advancing?"CLASSIFICA":"—"}</em></div>;})}</article>)}</section>{knockoutFixtures.length>0&&fixtureFeed(knockoutFixtures)}</>:(competition.type==="league"||competition.type==="state") ? <><article className="full-standings panel">
        ...
        {table.map((row,index)=>{const club=clubById(game,row.clubId),direct=!!league&&index<league.promotionPlaces,playoff=competition.id==="BRA-B"&&index>=2&&index<6,relegated=!!league&&league.relegationPlaces>0&&index>=table.length-league.relegationPlaces;return <div className={`full-standing ${row.clubId===game.userClubId?"is-us":""}`} key={row.clubId}><span><i className={direct?"promotion-zone":playoff?"continental":relegated?"danger-zone":competition.id==="BRA-A"&&index<8?"continental":""}/>{index+1}</span><span><Crest club={club} small/><strong>{club.name}</strong><small>{club.city}</small></span><span>{row.played}</span><span>{row.wins}</span><span>{row.draws}</span><span>{row.losses}</span><span>{row.gf}</span><span>{row.ga}</span><span>{row.gd}</span><b>{row.points}</b><span className={`destination ${direct?"up":relegated?"down":""}`}>{direct?"↑ ACESSO DIRETO":playoff?"PLAYOFF":relegated?"↓ DESCENSO":competition.type==="state"&&index===0?"TÍTULO":"—"}</span></div>;})}
      </article>{knockoutFixtures.length>0&&fixtureFeed(knockoutFixtures)}</>:fixtureFeed(competitionFixtures)}
...
function MarketView({ game, update }: { game: GameState; update: (next: GameState) => void }) {
  const [query,setQuery]=useState("");const [position,setPosition]=useState("Todos");const [mode,setMode]=useState<"ofertas"|"busca"|"elenco"|"transações">("ofertas");
  ...
  const ownPlayers=squadFor(game,game.userClubId).filter((player)=>!player.academy).sort((a,b)=>Number(b.listed)-Number(a.listed)||Number(b.starting)-Number(a.starting)||b.rating-a.rating);
  ...
  return (
    <>
      ...
      <div className="market-tabs"><button className={mode==="ofertas"?"active":""} onClick={()=>setMode("ofertas")}>OFERTADOS <span>{offered.length}</span></button><button className={mode==="busca"?"active":""} onClick={()=>setMode("busca")}>PESQUISA GLOBAL</button><button className={mode==="elenco"?"active":""} onClick={()=>setMode("elenco")}>MEU ELENCO</button><button className={mode==="transações"?"active":""} onClick={()=>setMode("transações")}>TRANSAÇÕES <span>{game.transferEvents.length}</span></button></div>
      ...
      {mode==="elenco"&&<article className="data-panel market-table panel"><div className="player-head"><span>JOGADOR</span><span>POS</span><span>IDADE</span><span>GER</span><span>FÍSICO</span><span>MORAL</span><span>VALOR</span><span>CONTRATO</span><span>AÇÃO</span></div>{ownPlayers.map((player)=><PlayerRow key={player.id} player={player} club={userClub(game)} action={<button className="buy-button" onClick={()=>update(togglePlayerListing(game,player.id))}>{player.listed?"RETIRAR DA LISTA":"COLOCAR À VENDA"}</button>}/>)}</article>}
