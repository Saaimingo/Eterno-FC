...
export function finishRound(game:GameState,plan:MatchPlan):GameState {
  ...
  const confidence=clamp(game.boardConfidence+(result==="V"?3:result==="D"?-3:0),5,99),matchesManaged=game.matchesManaged+1;let managerStatus=game.managerStatus;let vacancies=game.vacancies.filter((job)=>job.openedAt>=datePlusDays(userFixture.date,-120));let jobOffers=game.jobOffers;
...
export function acceptBid(game:GameState,bidId:string):GameState{const bid=game.incomingBids.find((item)=>item.id===bidId);return bid?sellPlayer(game,bid.playerId,bid.fee,bid.fromClubId):game;}
...
export function applyForJob(game:GameState,vacancyId:string):GameState{const vacancy=game.vacancies.find((item)=>item.id===vacancyId);if(!vacancy)return game;const club=clubById(game,vacancy.clubId);if(game.managerStatus==="unemployed"&&club.id===game.userClubId)return{...game,vacancies:game.vacancies.filter((item)=>item.id!==vacancyId),news:[{id:`job-former-${game.date}-${club.id}`,date:game.date,category:"carreira",title:`${club.name} seguirá outro caminho`,body:"A diretoria não reabriu a negociação após o fim do seu trabalho. Procure um novo projeto.",unread:true},...game.news]};if(game.managerReputation+8<vacancy.minimumReputation)return{...game,news:[{id:`job-no-${game.date}-${club.id}`,date:game.date,category:"carreira",title:`${club.name} escolhe outro treinador`,body:"Sua reputação ainda não corresponde à exigência da vaga.",unread:true},...game.news],vacancies:game.vacancies.filter((item)=>item.id!==vacancyId)};const offer={...vacancy,status:"offered" as const};return{...game,jobOffers:[offer,...game.jobOffers.filter((item)=>item.clubId!==club.id)],vacancies:game.vacancies.map((item)=>item.id===vacancyId?offer:item),news:[{id:`job-yes-${game.date}-${club.id}`,date:game.date,category:"carreira",title:`${club.name} apresenta uma proposta`,body:"A diretoria quer você no comando. A decisão é sua.",unread:true},...game.news]};}
export function acceptJob(game:GameState,offerId:string):GameState{const offer=[...game.jobOffers,...game.vacancies].find((item)=>item.id===offerId);if(!offer)return game;const club=clubById(game,offer.clubId);const records=game.managerRecord.map((record,index)=>index===game.managerRecord.length-1&&!record.toSeason?{...record,toSeason:game.season}:record);const next={...game,userClubId:club.id,managerStatus:"employed" as const,managerRecord:[...records,{clubId:club.id,fromSeason:game.season,matches:0,wins:0,trophies:0}],balance:club.balance,transferBudget:club.transferBudget,weeklyIncome:Math.round(club.reputation**2*165),weeklyExpenses:Math.round(club.reputation**2*128),boardConfidence:66,reputation:club.reputation,jobOffers:[],vacancies:game.vacancies.filter((item)=>item.clubId!==club.id),lastFive:[],news:[{id:`job-start-${game.date}-${club.id}`,date:game.date,category:"carreira",title:`Novo desafio: ${club.name}`,body:`Você assume o clube na ${leagueById(game,club.divisionId).name}. O calendário e a situação esportiva já estão em andamento.`,unread:true},...game.news]} as GameState;next.standings=refreshUserStandings(next);next.date=nextUserFixture(next)?.date??next.date;return next;}
export function resignJob(game:GameState):GameState{if(game.managerStatus==="unemployed")return game;return{...game,managerStatus:"unemployed",vacancies:[{id:`resign-${game.date}-${game.userClubId}`,clubId:game.userClubId,openedAt:game.date,minimumReputation:35,status:"open"},...game.vacancies],jobOffers:game.vacancies.filter((job)=>job.minimumReputation<=game.managerReputation).slice(0,2).map((job)=>({...job,status:"offered"})),news:[{id:`resign-news-${game.date}`,date:game.date,category:"carreira",title:"Você deixou o cargo",body:"Sua carreira continua. Consulte as vagas disponíveis e escolha o próximo projeto.",unread:true},...game.news]};}
