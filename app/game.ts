...
import { BRAZIL_2026, BRAZIL_STATE_CUP_SLOTS, brazilianDivisionSchedule, brazilianTransitionRules, regionalEligibleClubs } from "./rules/brazil-2026";
...
import { MATCH_ENGINE_VERSION, type MatchIntervention } from "./match-engine";
...
function makeSerieCOpening(competition:Competition,startDate:string):Fixture[]{
  const format=serieCFormatForSeason(competition.season);
  competition.currentStage="Primeira fase";
  if(format.firstStageGroupCount===1)return makeRoundRobin(competition,startDate,7,false).map((fixture)=>({...fixture,stage:"Primeira fase"}));
  const groups=createCompetitionGroups(competition,format.firstStageGroupCount);
  competition.groups=groups;competition.groupStage="Primeira fase";competition.groupAdvancing=format.qualifiedForAccessStage/format.firstStageGroupCount;
  return Object.values(groups).flatMap((clubIds,index)=>makeRoundRobin({...competition,id:`${competition.id}-primeira-${index+1}`,participantIds:clubIds},startDate,7,format.firstStageDoubleRound).map((fixture)=>({...fixture,id:`${competition.id}-${competition.season}-primeira-g${index+1}-r${fixture.round}-${fixture.homeId}-${fixture.awayId}`,competitionId:competition.id,stage:"Primeira fase"})));
}
...
function cupOpening(competition: Competition, clubs: Club[], date: string): Fixture[] {
  ...
  const lowerPower=2**Math.floor(Math.log2(sorted.length)); const bracketSize=lowerPower===sorted.length?lowerPower:lowerPower*2;const byes=Math.max(0,bracketSize-sorted.length); competition.pendingByes=sorted.slice(0,byes);
...
type SeasonQualification = { brazilPrimary: string[]; brazilSecondary: string[]; argentinaPrimary: string[]; argentinaSecondary: string[]; spainPrimary: string[]; italyPrimary: string[]; brazilCupProtected: string[]; brazilCupStateRoutes: Array<{stateCode:string;slots:number;rankedClubIds:string[]}>; brazilLeagueChampion: string; brazilLeagueRunnerUp: string; brazilCupChampion: string };
...
function buildSeason(clubs: Club[], season: number, qualification?: SeasonQualification) {
  ...
  LEAGUE_SEEDS.forEach((league)=>{
    ...
    competitions.push(competition); fixtures.push(...(league.id==="BRA-D"?makeGroupedRoundRobin(competition,openingDate,7,BRAZIL_2026.serieDFormat.groups):league.id==="BRA-C"?makeSerieCOpening(competition,openingDate):makeRoundRobin(competition,openingDate,league.id==="BRA-A"?8:league.id==="BRA-B"?7:SEASON_2026.league.roundIntervalDays,SEASON_2026.league.doubleRound)));
  });
  ...
  const superCupParticipants=resolveSuperCupParticipants({
    leagueChampionId:qualification?.brazilLeagueChampion??ranked("BRA-A")[0],
  ...
  const cups:Competition[]=[
    {...makeCompetition(BRAZIL_SUPER_CUP_2026.id,BRAZIL_SUPER_CUP_2026.name,BRAZIL_SUPER_CUP_2026.short,"cup","Brasil",season,[...superCupParticipants],dateForSeason(season,BRAZIL_SUPER_CUP_2026.matchDate)),prizeMoney:{champion:BRAZIL_SUPER_CUP_2026.championPrize,runnerUp:BRAZIL_SUPER_CUP_2026.runnerUpPrize}},
    ...regionalCups,
    brazilCup,
...
function transitionTable(game:GameState,leagueId:string){
  const competition=competitionById(game,leagueId),stage=leagueId==="BRA-C"?"Primeira fase":undefined,fixtures=game.fixtures.filter((fixture)=>fixture.competitionId===leagueId&&(!stage||fixture.stage===stage));
  return calculateStandings(competition.participantIds.map((id)=>clubById(game,id)),fixtures);
}
...
export function buildLegacyMatchPlan(game:GameState,fixture:Fixture):MatchPlan {
  ...
  return{fixtureId:fixture.id,engineSource:"legacy",engineVersion:"legacy-v2",homeGoals,awayGoals,durationMinutes:90,decisionMethod:homeGoals===awayGoals?"draw":"regulation",winnerTeamId:homeGoals>awayGoals?fixture.homeId:awayGoals>homeGoals?fixture.awayId:undefined,events:presentedEvents,phases,homePossession,homeShots:Math.max(homeGoals+2,7+Math.round((homeStrength-awayStrength)/5+random()*5)),awayShots:Math.max(awayGoals+2,6+Math.round((awayStrength-homeStrength)/5+random()*5)),homeCorners:0,awayCorners:0,homeCards:events.filter((event)=>event.type==="card"&&event.teamId===fixture.homeId).length,awayCards:events.filter((event)=>event.type==="card"&&event.teamId===fixture.awayId).length};
}

export function buildMatchPlan(game:GameState,fixture:Fixture,interventions:readonly MatchIntervention[]=[]):MatchPlan {
  const legacy=buildLegacyMatchPlan(game,fixture);
  try{
    const candidate=simulateVNextFixture(game,fixture,interventions),shadow=compareShadowMatch([legacy.homeGoals,legacy.awayGoals],[legacy.homeShots,legacy.awayShots],legacy.homePossession,candidate);
...
export function matchPlanPrefixIsStable(before:MatchPlan,after:MatchPlan,throughMinute:number){
  const prefix=(plan:MatchPlan)=>plan.events.filter((event)=>event.minute<=throughMinute).map((event)=>({
    id:event.id,sequence:event.sequence,minute:event.minute,minuteLabel:event.minuteLabel,type:event.type,teamId:event.teamId,
    playerId:event.playerId,targetPlayerId:event.targetPlayerId,assistPlayerId:event.assistPlayerId,text:event.text,detail:event.detail,outcome:event.outcome,scoreAfter:event.scoreAfter,
  }));
  return JSON.stringify(prefix(before))===JSON.stringify(prefix(after));
}
...
function fixtureWinner(fixture:Fixture){
  if(fixture.winnerId===fixture.homeId||fixture.winnerId===fixture.awayId)return fixture.winnerId;
  if((fixture.homeGoals??0)>(fixture.awayGoals??0))return fixture.homeId;
  if((fixture.awayGoals??0)>(fixture.homeGoals??0))return fixture.awayId;
  return undefined;
}
...
function advanceCups(game:GameState,fixtures:Fixture[],competitions:Competition[]) {
  ...
  nextCompetitions.filter((competition)=>(competition.type==="cup"||competition.type==="continental")&&!competition.complete&&!competition.formatId).forEach((competition)=>{
    ...
    const winners=stageFixtures.map((fixture)=>fixtureWinner(fixture)??fixture.awayId);const nextTeams=[...competition.pendingByes,...winners];competition.pendingByes=[];
...
function advanceBrazilCup(game:GameState,fixtures:Fixture[],competitions:Competition[]){
  ...
  const twoLegged=BRAZIL_CUP_2026_FORMAT.twoLeggedStages.includes(competition.currentStage as typeof BRAZIL_CUP_2026_FORMAT.twoLeggedStages[number]),winners=twoLegged?resolveTwoLeggedTies(stageFixtures,(first,second)=>squadStrength(game,first)>=squadStrength(game,second)?first:second).winners:stageFixtures.map((fixture)=>fixtureWinner(fixture)??fixture.awayId);
...
function advanceBrazilianLeagueStages(game:GameState,fixtures:Fixture[],competitions:Competition[]){
  ...
  const serieC=nextCompetitions.find((competition)=>competition.id==="BRA-C");
  if(serieC&&!serieC.complete){
    const stageFixtures=nextFixtures.filter((fixture)=>fixture.competitionId===serieC.id&&fixture.stage===serieC.currentStage);
    if(stageFixtures.length&&stageFixtures.every((fixture)=>fixture.played)){
      if(serieC.currentStage==="Primeira fase"){
        const ranked=serieC.groups
          ?Object.values(serieC.groups).flatMap((clubIds)=>calculateStandings(clubIds.map((id)=>clubById(game,id)),stageFixtures).slice(0,4)).sort((left,right)=>right.points-left.points||right.wins-left.wins||right.gd-left.gd||right.gf-left.gf)
          :calculateStandings(serieC.participantIds.map((id)=>clubById(game,id)),stageFixtures);
        const accessGroups=splitSerieCAccessGroups(ranked.slice(0,8).map((row)=>row.clubId));
        serieC.groups=accessGroups;serieC.groupStage="Quadrangulares de acesso";serieC.groupAdvancing=2;serieC.currentStage="Quadrangulares de acesso";
        const lastDate=stageFixtures.reduce((latest,fixture)=>fixture.date>latest?fixture.date:latest,stageFixtures[0].date);
        nextFixtures.push(...Object.values(accessGroups).flatMap((clubIds,index)=>makeRoundRobin({...serieC,id:`${serieC.id}-acesso-${index+1}`,participantIds:clubIds},datePlusDays(lastDate,7),7,true).map((fixture)=>({...fixture,id:`${serieC.id}-${serieC.season}-acesso-g${index+1}-r${fixture.round}-${fixture.homeId}-${fixture.awayId}`,competitionId:serieC.id,stage:"Quadrangulares de acesso"}))));
      }else if(serieC.currentStage==="Quadrangulares de acesso"){
        const groupTables=Object.values(serieC.groups??{}).map((clubIds)=>calculateStandings(clubIds.map((id)=>clubById(game,id)),stageFixtures));
        serieC.promotedClubIds=groupTables.flatMap((table)=>table.slice(0,2).map((row)=>row.clubId));serieC.currentStage="Final";
        const finalists=groupTables.map((table)=>table[0].clubId),lastDate=stageFixtures.reduce((latest,fixture)=>fixture.date>latest?fixture.date:latest,stageFixtures[0].date);
        nextFixtures.push(...buildTwoLeggedTies({competitionId:serieC.id,season:serieC.season,stage:"Final",round:Math.max(...stageFixtures.map((fixture)=>fixture.round))+1,firstLegDate:datePlusDays(lastDate,7),pairs:pairTeams(finalists)}));
      }else if(serieC.currentStage==="Final"){
        serieC.championId=resolveTwoLeggedTies(stageFixtures,tieBreaker).winners[0];serieC.complete=true;serieC.currentStage="Campeão";
      }
    }
  }
  ...
  if(serieD&&!serieD.complete){
    ...
    if(serieD.currentStage==="Fase de grupos"){
      ...
      if(serieD.currentStage==="Semifinais e playoffs"){
        const semifinals=stageFixtures.filter((fixture)=>fixture.tieId?.includes("-semi-")),playoffs=stageFixtures.filter((fixture)=>fixture.tieId?.includes("-playoff-"));const semifinalWinners=resolveTwoLeggedTies(semifinals,tieBreaker).winners,playoffWinners=resolveTwoLeggedTies(playoffs,tieBreaker).winners;serieD.promotedClubIds=[...(serieD.promotedClubIds??[]),...playoffWinners];serieD.currentStage="Final";const firstDate=datePlusDays(stageFixtures.reduce((latest,fixture)=>fixture.date>latest?fixture.date:latest,stageFixtures[0].date),7),finalFixtures=buildTwoLeggedTies({competitionId:serieD.id,season:serieD.season,stage:"Final",round:17,firstLegDate:firstDate,pairs:pairTeams(semifinalWinners)}).map((fixture)=>({...fixture,id:fixture.id.replace("-17-","-final-"),tieId:fixture.tieId?.replace("-17-","-final-")}));nextFixtures.push(...finalFixtures);
        ...
        if(serieD.currentStage==="Quartas"){serieD.promotedClubIds=resolved.winners;serieD.currentStage="Semifinais e playoffs";const semifinalFixtures=buildTwoLeggedTies({competitionId:serieD.id,season:serieD.season,stage:serieD.currentStage,round:15,firstLegDate:datePlusDays(lastDate,7),pairs:pairTeams(resolved.winners)}).map((fixture)=>({...fixture,id:fixture.id.replace("-15-","-semi-"),tieId:fixture.tieId?.replace("-15-","-semi-")})),playoffFixtures=buildTwoLeggedTies({competitionId:serieD.id,season:serieD.season,stage:serieD.currentStage,round:15,firstLegDate:datePlusDays(lastDate,7),pairs:pairTeams(resolved.losers)}).map((fixture)=>({...fixture,id:fixture.id.replace("-15-","-playoff-"),tieId:fixture.tieId?.replace("-15-","-playoff-")}));nextFixtures.push(...semifinalFixtures,...playoffFixtures);}else{const nextStage=stageOrder[serieD.currentStage];if(nextStage){serieD.currentStage=nextStage;nextFixtures.push(...buildTwoLeggedTies({competitionId:serieD.id,season:serieD.season,stage:nextStage,round:stageFixtures[0].round+2,firstLegDate:datePlusDays(lastDate,7),pairs:pairTeams(resolved.winners)}));}}
      }
    }
  }
  return{fixtures:nextFixtures,competitions:nextCompetitions};
...
function advanceCompetitions(game:GameState,fixtures:Fixture[],competitions:Competition[]){const nationalCup=advanceBrazilCup(game,fixtures,competitions),regionals=advanceRegionalCups({...game,fixtures:nationalCup.fixtures,competitions:nationalCup.competitions},nationalCup.fixtures,nationalCup.competitions),cups=advanceCups({...game,fixtures:regionals.fixtures,competitions:regionals.competitions},regionals.fixtures,regionals.competitions),leagues=advanceBrazilianLeagueStages({...game,fixtures:cups.fixtures,competitions:cups.competitions},cups.fixtures,cups.competitions);return{...leagues,fixtures:resolveClubDateConflicts(leagues.fixtures)};}
...
export function finishRound(game:GameState,plan:MatchPlan):GameState {
  ...
  let fixtures=game.fixtures.map((fixture)=>{if(fixture.id===userFixture.id)return{...fixture,played:true,homeGoals:plan.homeGoals,awayGoals:plan.awayGoals,winnerId:plan.winnerTeamId,decision:plan.decisionMethod&&plan.decisionMethod!=="draw"?plan.decisionMethod:undefined,shootoutHomeGoals:plan.shootoutScore?.[0],shootoutAwayGoals:plan.shootoutScore?.[1]};if(!fixture.played&&fixture.date<=userFixture.date&&fixture.homeId!==game.userClubId&&fixture.awayId!==game.userClubId)return playFixture(game,fixture,"world");return fixture;});
  ...
  const decisionText=plan.decisionMethod==="penalties"?` após ${plan.shootoutScore?.[0]??0} a ${plan.shootoutScore?.[1]??0} nos pênaltis`:plan.decisionMethod==="extra_time"?" após a prorrogação":"";
  const news:NewsItem[]=[{id:`result-${userFixture.id}`,date:userFixture.date,category:"competição",title:`${userClub(game).name} ${userGoals} x ${opponentGoals} ${opponent.name}${decisionText}`,body:`${competitionById(game,userFixture.competitionId).short}: ${result==="V"?"vitória importante":result==="E"?"empate equilibrado":"derrota que exige reação"}.`,unread:true},...ai.events.slice(0,1).map((event)=>({id:`news-${event.id}`,date:event.date,category:"mercado" as const,title:`${event.playerName} muda de clube`,body:`Negociação fechada por ${formatMoney(event.fee)}. O mercado segue ativo enquanto as competições acontecem.`,unread:true}))];
...
function qualificationFromSeason(game:GameState,tables: Map<string,Standing[]>): SeasonQualification {
  ...
  const champions=[game.competitions.find((competition)=>competition.id==="BRA-C")?.championId,game.competitions.find((competition)=>competition.id==="BRA-D")?.championId,game.competitions.find((competition)=>competition.id==="REGIONAL-NE")?.championId,game.competitions.find((competition)=>competition.id==="REGIONAL-NCO")?.championId].filter((id):id is string=>Boolean(id));
  const brazilCupStateRoutes=Object.entries(BRAZIL_STATE_CUP_SLOTS).map(([stateCode,slots])=>{const competitionId=`STATE-${stateCode}`,competition=game.competitions.find((item)=>item.id===competitionId),fixtures=game.fixtures.filter((fixture)=>fixture.competitionId===competitionId),rankedClubIds=competition?calculateStandings(competition.participantIds.map((id)=>clubById(game,id)),fixtures).map((row)=>row.clubId):[];return{stateCode,slots,rankedClubIds};});
  return {brazilPrimary:brazil.slice(0,8),brazilSecondary:brazil.slice(8,16),argentinaPrimary:argentina.slice(0,8),argentinaSecondary:argentina.slice(8,16),spainPrimary:spain.slice(0,8),italyPrimary:italy.slice(0,8),brazilCupProtected:[...new Set(champions)].slice(0,4),brazilCupStateRoutes,brazilLeagueChampion:brazil[0],brazilLeagueRunnerUp:brazil[1],brazilCupChampion:game.competitions.find((competition)=>competition.id==="COPA-BR")?.championId??brazil[2]};
}
...
export function startNextSeason(game:GameState):GameState {
  const completed=completeWorldSeason(game);const snapshot={...game,fixtures:completed.fixtures,competitions:completed.competitions} as GameState;const tables=new Map<string,Standing[]>();game.leagues.forEach((league)=>tables.set(league.id,transitionTable(snapshot,league.id)));
